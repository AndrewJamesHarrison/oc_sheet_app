...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<List<ViewPort>> reloadViewPort() async {
  ...
  {
    return new Row(children: v.groups.map((item) => getGroupWidget(item)).toList());
  ...
  {
    return new Row(children: <Widget>[Text(p.name), Text(p.value)]);
  }

  @override
  Widget build(BuildContext context) {
    reloadViewPort().then((response) => viewPorts = response);
    testConnection().then((response) => test = response);
